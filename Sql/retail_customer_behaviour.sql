create database retail_customer_behaviour_db
...
select top 5 item_purchased, round(avg(cast(review_rating as float)),2) as "Average Product Rating" from customer group by item_purchased 
order by avg(review_rating) desc 


--Q4. Compare the average Purchase Amounts between Standard and Express Shipping. 
select shipping_type, round(avg(cast(purchase_amount as float)),2) from customer
where shipping_type in ('Standard','Express')
group by shipping_type;

--Q5. Do subscribed customers spend more? Compare average spend and total revenue 
--between subscribers and non-subscribers.
select subscription_status,count(customer_id) as total_customers,round(avg(cast(purchase_amount as float)),2) as avg_spend,
round(sum(cast(purchase_amount as float)),2) as total_revenue from customer group by subscription_status 
...
--Q11. Checks if discounts actually drive higher spending ?
SELECT discount_applied, AVG(CAST(purchase_amount AS FLOAT)) AS [avg_purchase_amount] FROM customer GROUP BY discount_applied;

--Q12. Finds seasonal quality issues by looking for low ratings?
SELECT season, category, AVG(CAST(review_rating AS FLOAT)) AS [avg_rating] FROM customer GROUP BY season, category
HAVING AVG(CAST(review_rating AS FLOAT)) < 3.ORDER BY [avg_rating] ASC;